<section class="w-full">
    ...
    <x-user::settings.layout :heading="__('Two Factor Authentication')" :subheading="__('Manage your two-factor authentication settings')">
        <div class="mx-auto flex w-full flex-col space-y-6 text-sm" wire:cloak>
            @if ($twoFactorEnabled)
                <div class="space-y-4">
                    ...
                    <div class="flex justify-start">
                        <flux:button icon:variant="outline" icon="shield-exclamation" variant="danger"
                            wire:click="disable">
                            {{ __('Disable 2FA') }}
                        </flux:button>
                    </div>
            ...
            @else
                <div class="space-y-4">
                    ...
                    <flux:button icon:variant="outline" icon="shield-check" variant="primary" wire:click="enable">
                        {{ __('Enable 2FA') }}
                    </flux:button>
                </div>
            @endif
    ...
    <flux:modal @close="closeModal" class="md:min-w-md max-w-md" name="two-factor-setup-modal" wire:model="showModal">
        <div class="space-y-6">
            ...
            @else
                @error('setupData')
                    <flux:callout heading="{{ $message }}" icon="x-circle" variant="danger" />
                @enderror
                ...
                <div class="flex justify-center">
                    ...
                    @else
                        <div class="flex h-full items-center justify-center p-4" x-data>
                            <div :style="($flux.appearance === 'dark' || ($flux.appearance === 'system' && $flux.dark)) ?
            ...
            <div>
                <flux:button :disabled="$errors->has('setupData')" class="w-full" variant="primary"
            ...
            <div class="space-y-4">
                ...
                <div class="flex items-center space-x-2" x-data="{
                    ...
                    @else
                        <input class="w-full bg-transparent p-3 text-stone-900 outline-none dark:text-stone-100"
                            readonly type="text" value="{{ $manualSetupKey }}" />

                        <button @click="copy()"
                            class="cursor-pointer border-l border-stone-200 px-3 transition-colors dark:border-stone-600">
                            <flux:icon.document-duplicate variant="outline" x-show="!copied"></flux:icon>
                                <flux:icon.check class="text-green-500" variant="solid" x-show="copied">
                                    </flux:icon>
                        </button>
                    @endempty
                </div>
